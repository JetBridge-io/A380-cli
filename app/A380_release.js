...
function run (args, main_callback) {
  ...
  var A380_config = null
  try {
    A380_config = jsonfile.readFileSync(config.BUNDLE_BUS_CONFIG)
  } catch (e) {
  ...
  var appkey = A380_config[config.ACCESS_KEY]
  if (args.length === 1) { // User entered 'A380 release'
    showSupportOSList(appkey, main_callback)
  } else { // User entered 'A380 release <os>'
    if (args[1] === '-h') {
      help()
      main_callback()
      return
